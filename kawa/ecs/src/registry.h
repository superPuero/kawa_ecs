...
namespace kawa
{
	namespace ecs
	{
		...
		public:
			...
			inline entity_id entity(Args&&...args) noexcept
			{
				entity_id id = _entity_manager.get_new();
			...
			{
				KAWA_DEBUG_EXPAND(_validate_entity(entity));
				KAWA_ASSERT_MSG(!_query_level, "usage of registry::destroy() inside queries is forbidden");
			...
			{
				//KAWA_DEBUG_EXPAND(_validate_entity(entity));
				if (entity == ecs::nullent)
				{
					return false;
				}

				return _entity_manager.alive(entity);
			}
			...
			inline void query_info_with(entity_id entity, Fn&& fn, Params&&...params) noexcept
			{
				KAWA_DEBUG_EXPAND(_validate_entity(entity));
			...
			inline void query_info(Fn&& fn, Params&&...params) noexcept
			{
				_query_level++;
			...
			inline void query(Fn&& fn, Params&&...params) noexcept
			{
				...
				if constexpr (query_traits::is_empty)
				{
